...
import { z } from "zod";
import { buildOccurrenceDocx } from "./pdf.service.js";
...
export async function getOccurrenceDocxHandler(req: Request, res: Response) {
  try {
    ...
    const result = await buildOccurrenceDocx({
      occurrenceId,
      force,
      maxPhotos: 20,
    ...
    console.error("[getOccurrenceDocxHandler] erro inesperado:", err);
