...
export async function getExcessoParadaReportPdfHandler(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const storagePath = `telemetry/${id}-excesso-parada.pdf`;
    ...
    const points = (row.points as Parameters<typeof extractExcessos>[0]) ?? [];
    const excessos = extractExcessos(points, tempoMap);
    ...
    const url = await createSignedUrl(BUCKET, storagePath, query.ttl);
    return res.json({ url, cached: false, excessos: excessos.length });
  } catch (err) {
    next(err);
  }
}
